"""Asynchronous database client abstraction for MySQL and PostgreSQL."""
...
class DBConfig:
    """DB settings used by connection pool."""
...
class FreeRadiusDBClient(ABC):
    """Abstract repository-style access to FreeRADIUS tables."""
    ...
    async def get_user_entity(self, username: str) -> dict[str, Any] | None:
        """Return entity metadata from userinfo for a username."""
        pass
...
class FreeRadiusMySQLClient(FreeRadiusDBClient):
    """Repository-style access to FreeRADIUS tables via MySQL."""
    ...
    async def _ensure_entity_type_description_column(self) -> None:
        """Ensure userinfo has an optional description column."""
    ...
    async def _set_entity_type(self, username: str, entity_type: str, description: str | None = None) -> None:
        sql = (
            "INSERT INTO userinfo (username, entity_type, description) VALUES (%s, %s, %s) "
    ...
    async def create_user(
        ...
        existing_type = await self.fetch_one(
            "SELECT 1 AS ok FROM userinfo WHERE username=%s LIMIT 1",
            (username,),
        )
    ...
    async def update_user(
        ...
        entity_row = await self.fetch_one(
            "SELECT entity_type FROM userinfo WHERE username=%s", (username,)
        )
        ...
        statements: list[tuple[str, tuple[Any, ...]]] = [
            ...
            ("UPDATE userinfo SET description=%s WHERE username=%s", (description, username)),
        ]
    ...
    async def get_user_entity(self, username: str) -> dict[str, Any] | None:
        ...
        return await self.fetch_one(
            ...
            "FROM userinfo WHERE username=%s LIMIT 1",
            (username,),
        )
    ...
    async def delete_user(self, username: str) -> None:
        ...
        await self.transaction(
            [
                ...
                ("DELETE FROM radreply WHERE username=%s", (username,)),
                ("DELETE FROM radusergroup WHERE username=%s", (username,)),
                ("DELETE FROM userinfo WHERE username=%s", (username,)),
            ]
        )
    ...
    async def set_user_enable(self, username: str, enabled: bool) -> None:
        ...
        entity_row = await self.fetch_one(
            "SELECT entity_type FROM userinfo WHERE username=%s LIMIT 1",
            (username,),
        )
        if not entity_row:
    ...
    async def sync_users(self) -> dict[str, int]:
        ...
        rows = await self.fetch_one(
            "SELECT COUNT(*) AS total FROM userinfo et "
            "LEFT JOIN radcheck rc ON rc.username=et.username "
            "WHERE rc.username IS NULL"
        )
        dangling = int(rows["total"]) if rows else 0
        if dangling:
            _LOGGER.warning("Found %s entries in userinfo without radcheck rows", dangling)
        return {"dangling_type_rows": dangling}


class FreeRadiusPostgreSQLClient(FreeRadiusDBClient):
    """Repository-style access to FreeRADIUS tables via PostgreSQL."""
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            raise FreeRadiusDBError(f"Could not connect to PostgreSQL: {err}") from err
    ...
    async def validate_schema(self) -> None:
        ...
        row = await self.fetch_one(sql, (TABLE_ENTITY_TYPE,))
        if not row or int(row["total"]) == 0:
            raise FreeRadiusDBError(
                f"Required table '{TABLE_ENTITY_TYPE}' does not exist in schema 'public'."
            )
    ...
    async def fetch_all(self, sql: str, params: tuple[Any, ...] = ()) -> list[dict[str, Any]]:
        """Run SELECT query and return all rows as dicts."""
        if self._pool is None:
            raise FreeRadiusDBError("PostgreSQL pool is not initialized")

        try:
            # Convert MySQL placeholders (%) to PostgreSQL placeholders ($1, $2, etc.)
            pg_sql = self._convert_sql_placeholders(sql)
            async with self._pool.acquire() as conn:
                rows = await conn.fetch(pg_sql, *params)
                return [self._normalize_postgres_row(dict(row)) for row in rows]
        except Exception as err:  # noqa: BLE001
            raise FreeRadiusDBError(str(err)) from err

    @staticmethod
    def _normalize_postgres_row(row: dict[str, Any]) -> dict[str, Any]:
        """Convert asyncpg-specific scalar types to JSON-serializable values."""
    ...
    def _convert_sql_placeholders(self, sql: str) -> str:
        ...
        def replace_placeholder(match):
            ...
            counter += 1
            return result
        return re.sub(r'%s', replace_placeholder, sql)

    def _convert_sql_insert_duplicate_key(self, sql: str) -> str:
        """Convert MySQL INSERT ... ON DUPLICATE KEY UPDATE to PostgreSQL ON CONFLICT."""
        # This is a simplified conversion; adjust as needed for specific SQL patterns
        import re
        
        # Pattern: INSERT INTO table (cols) VALUES (...) ON DUPLICATE KEY UPDATE col1=VALUES(col1), col2=VALUES(col2)
        match = re.search(
            r'INSERT INTO (\w+)\s*\((.*?)\)\s*VALUES\s*\((.*?)\)\s*ON DUPLICATE KEY UPDATE\s*(.*?)$',
            sql,
        ...
        table = match.group(1)
        cols = match.group(2)
        values_placeholder = match.group(3)
        updates = match.group(4)
        
        # Assume primary key is the first column
        col_list = [c.strip() for c in cols.split(',')]
        pk_col = col_list[0] if col_list else 'id'
        
        # Build ON CONFLICT clause
        update_pairs = []
        for update_part in updates.split(','):
            update_part = update_part.strip()
            if 'VALUES(' in update_part:
                col_name = update_part.split('=')[0].strip()
                update_pairs.append(f"{col_name}=EXCLUDED.{col_name}")
            else:
                update_pairs.append(update_part)
        
        update_clause = ', '.join(update_pairs)
        
        return (
            f"INSERT INTO {table} ({cols}) VALUES ({values_placeholder}) "
    ...
    async def get_groups(self) -> list[dict[str, Any]]:
        ...
        sql = (
            "SELECT r.id, r.groupname, r.attribute, r.op, r.value, "
            "COALESCE(c.value, $1) as auth_type "
            "FROM radgroupreply r "
            "LEFT JOIN radgroupcheck c ON r.groupname = c.groupname "
            "AND c.attribute = $2 "
            "ORDER BY r.groupname, r.id"
    ...
    async def get_users(
        ...
        sql = (
            ...
            "WHEN rc.attribute=%s AND LOWER(TRIM(rc.value))=LOWER(%s) THEN %s "
            "ELSE NULL END), %s) AS enable, "
            "COALESCE(STRING_AGG(DISTINCT g.groupname, ', ' ORDER BY g.groupname), '') AS groupnames "
            "FROM userinfo et "
            "LEFT JOIN radcheck rc ON rc.username=et.username "
            "LEFT JOIN radusergroup g ON g.username=et.username "
            f"{where_clause} "
    ...
    async def _set_entity_type(self, username: str, entity_type: str, description: str | None = None) -> None:
        ...
        sql = (
            "INSERT INTO userinfo (username, entity_type, description) "
            "VALUES (%s, %s, %s) "
